...
import { BaseStorageService, metricsService } from 'everytriv-shared/services/storage';
import {
...
export class ServerStorageService extends BaseStorageService implements UnifiedStorageService {
	...
	async clear(): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			const prefixedKeys = keysResult.data.map(key => this.getPrefixedKey(key));
	...
	async getKeys(): Promise<StorageOperationResult<string[]>> {
		...
		try {
			...
			const unprefixedKeys = keys.map(key => key.replace(this.config.prefix, ''));
