...
import { GenericDataValue } from 'everytriv-shared/types/data.types';
import {
...
import { createTimedResult, formatStorageError, trackOperationWithTiming } from 'everytriv-shared/utils/storage.utils';
...
export class CacheService implements UnifiedStorageService, OnModuleDestroy {
	private memoryCache = new Map<string, { value: GenericDataValue; expiry: number | null }>();
	...
	async set<T>(key: string, value: T, ttl?: number): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			if (this.useRedis && this.redisClient) {
				await this.setRedis(prefixedKey, value as GenericDataValue, ttl);
			} else {
				this.setMemory(prefixedKey, value as GenericDataValue, ttl);
			}

			this.logger.cacheSet(key, {
				key,
				ttl: ttl,
			});
			...
			this.logger.cacheError('set', key, {
	...
	async get<T>(key: string): Promise<StorageOperationResult<T | null>> {
		...
		try {
			...
			this.logger.cacheError('get', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
	...
	async getOrSet<T>(key: string, factory: () => Promise<T>, ttl?: number): Promise<T> {
		try {
			...
			this.logger.cacheError('getOrSet', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			throw error;
		}
	 ...
	 */
	async mget<T = GenericDataValue>(keys: string[]): Promise<Array<T | null>> {
		try {
			if (this.useRedis && this.redisClient) {
			...
			this.logger.cacheError('mget', keys.join(','), {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
	 ...
	 */
	async mset(keyValues: Array<{ key: string; value: GenericDataValue; ttl?: number }>): Promise<void> {
		try {
			if (this.useRedis && this.redisClient) {
			...
			this.logger.cacheSet('multiple', {
				keysCount: keyValues.length,
			});
		} catch (error) {
			this.logger.cacheError('mset', 'multiple', {
				error: error instanceof Error ? error.message : 'Unknown error',
				keysCount: keyValues.length,
			});
		}
	...
	async increment(key: string, amount: number = 1): Promise<number> {
		try {
			...
			this.logger.cacheError('increment', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			return 0;
		}
	...
	async delete(key: string): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			this.logger.cacheError('delete', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			return createTimedResult<void>(false, undefined, formatStorageError(error), startTime, 'cache');
		}
	...
	async exists(key: string): Promise<StorageOperationResult<boolean>> {
		...
		try {
			...
			this.logger.cacheError('exists', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
	...
	async setTTL(key: string, ttl: number): Promise<boolean> {
		try {
			...
			this.logger.cacheError('setTTL', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			return false;
		}
	...
	async getTTL(key: string): Promise<number> {
		try {
			...
			this.logger.cacheError('getTTL', key, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			return -1;
		}
	...
	async clear(): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			this.logger.system('Cache cleared', {});
	...
	async getStats(): Promise<StorageOperationResult<StorageStats>> {
		...
		try {
			...
			this.logger.cacheError('getStats', '', {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
	...
	async invalidate(pattern: string): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			this.logger.cacheError('invalidatePattern', pattern, {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
			return createTimedResult<void>(false, undefined, formatStorageError(error), startTime, 'cache');
		}
	...
	async getKeys(): Promise<StorageOperationResult<string[]>> {
		...
		try {
			...
			this.logger.cacheError('getKeys', '', {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
	...
	async cleanup(options?: StorageCleanupOptions): Promise<StorageOperationResult<void>> {
		...
		try {
			...
			this.logger.cacheInfo('Cache cleanup completed', {
				cleanedCount,
				totalSize,
				dryRun,
			...
			this.logger.cacheError('cleanup', '', {
				error: error instanceof Error ? error.message : 'Unknown error',
	 ...
	 */
	private setMemory(key: string, value: GenericDataValue, ttl?: number): void {
	...
	private mgetMemory<T = GenericDataValue>(keys: string[]): Array<T | null> {
		return keys.map(key => this.getMemory<T>(key));
	}

	private msetMemory(keyValues: Array<{ key: string; value: GenericDataValue; ttl?: number }>): void {
	...
	// Redis cache methods
	private async setRedis(key: string, value: GenericDataValue, ttl?: number): Promise<void> {
	...
	private async mgetRedis<T = GenericDataValue>(keys: string[]): Promise<Array<T | null>> {
		const values = await this.redisClient!.mget(keys);
		return values.map(value => (value ? JSON.parse(value) : null));
	}

	private async msetRedis(keyValues: Array<{ key: string; value: GenericDataValue; ttl?: number }>): Promise<void> {
	...
	onModuleDestroy() {
		try {
			this.clearMemory();
			this.logger.system('Cache service destroyed', {});
		} catch (error) {
			this.logger.systemError('Failed to destroy cache service', {
				error: error instanceof Error ? error.message : 'Unknown error',
			});
		}
	}
