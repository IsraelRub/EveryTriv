...
export class PointCalculationService {
	 ...
	 */
	calculateAnswerPoints(difficulty: DifficultyLevel, timeSpent: number, streak: number, isCorrect: boolean): number {
	 ...
	 */
	calculateBonusPoints(baseScore: number, difficulty: DifficultyLevel, timeBonus: number = 1): number {
	...
	calculateDailyLimit(
		...
		if (lastResetTime) {
			...
			if (lastReset.toDateString() === now.toDateString()) {
				remainingPoints = Math.max(0, dailyLimit - (dailyLimit - currentBalance.freeQuestions));
			}
		}
