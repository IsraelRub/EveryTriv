...
import { GAME_MODE_DEFAULTS, GameMode as GameModeEnum, VALID_GAME_MODES, VALIDATION_LIMITS } from '@shared/constants';

import { ButtonVariant, ComponentSize, DEFAULT_GAME_MODE } from '../../constants';
...
export default function GameMode({ isVisible, onSelectMode, onCancel }: GameModeUIProps) {
	...
	const [timeLimit, setTimeLimit] = useState<number | undefined>(() => GAME_MODE_DEFAULTS[DEFAULT_GAME_MODE].timeLimit);
	const [questionLimit, setQuestionLimit] = useState<number>(() => GAME_MODE_DEFAULTS[DEFAULT_GAME_MODE].questionLimit);
			...
			>
				...
				<div className='space-y-6'>
					...
					<div className='flex flex-col gap-3'>
						...
						<Button
							variant={mode === GameModeEnum.QUESTION_LIMITED ? ButtonVariant.PRIMARY : ButtonVariant.GHOST}
						...
						>
							<span>
								<Icon name='list' size={ComponentSize.SM} className='mr-1' /> Question Limited
							</span>
						...
						{/* Time Limited Mode */}
						<Button
							variant={mode === GameModeEnum.TIME_LIMITED ? ButtonVariant.PRIMARY : ButtonVariant.GHOST}
							className='w-full justify-between items-center flex'
							onClick={() => {
						...
						>
							<span>
								<Icon name='timer' size={ComponentSize.SM} className='mr-1' /> Time Limited
							</span>
						...
						{/* Unlimited Mode */}
						<Button
							variant={mode === GameModeEnum.UNLIMITED ? ButtonVariant.PRIMARY : ButtonVariant.GHOST}
							className='w-full justify-between items-center flex'
							onClick={() => {
						...
						>
							<span>
								<Icon name='infinity' size={ComponentSize.SM} className='mr-1' /> Unlimited
							</span>
						...
						{mode === GameModeEnum.UNLIMITED && (
							<motion.div variants={fadeInUp} initial='hidden' animate='visible' className='pl-8 text-white/70 text-sm'>
					...
					<div className='flex gap-3 justify-end'>
						...
						<Button
							...
							onClick={() => {
								const defaults = GAME_MODE_DEFAULTS[mode];
								// Use state value if it's valid for current mode, otherwise use defaults
								// This prevents stale values from previous mode (e.g., 999 from unlimited mode)
								const finalQuestionLimit =
									mode === GameModeEnum.QUESTION_LIMITED
										? questionLimit !== GAME_MODE_DEFAULTS[GameModeEnum.UNLIMITED].questionLimit &&
											questionLimit !== GAME_MODE_DEFAULTS[GameModeEnum.TIME_LIMITED].questionLimit
											? questionLimit
