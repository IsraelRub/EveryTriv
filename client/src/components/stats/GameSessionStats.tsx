					...
					<div className='text-sm text-slate-300 space-y-2'>
						<div className='flex justify-between'>
							...
							<span className='text-white'>
								{timeElapsed.days > 0 ? `${timeElapsed.days}d ago` : 
								 timeElapsed.hours > 0 ? `${timeElapsed.hours}h ago` :
								 timeElapsed.minutes > 0 ? `${timeElapsed.minutes}m ago` : 'Just now'}
							</span>
						</div>
						<div className='flex justify-between'>
							<span>Correct Answers:</span>
							<span className='text-white'>{session.correctAnswers}/{session.totalQuestions}</span>
						</div>
						<div className='flex justify-between'>
						...
						<div className='flex justify-between'>
							...
							<span className={`font-medium ${session.endTime ? 'text-green-400' : 'text-yellow-400'}`}>
