import { FileQuestion, GamepadIcon, TrendingUp, Trophy } from 'lucide-react';

import type { UserProgressTopic } from '@shared/types';
import { formatForDisplay } from '@shared/utils';

import { CHART_HEIGHTS, SKELETON_WIDTHS, StatCardVariant, TextColor, VariantBase } from '@/constants';
import {
	Badge,
	Card,
	CardContent,
	...
	if (progressLoading) {
		return (
			<div className='space-y-4'>
				<Skeleton className={`h-96 ${SKELETON_WIDTHS.FULL}`} />
			</div>
		);
	}
		...
		<div className='space-y-6'>
			...
			{userProgress.data.timeline && userProgress.data.timeline.length > 0 && (
				<Card>
					...
					<CardContent>
						<TrendChart data={userProgress.data.timeline} height={CHART_HEIGHTS.LARGE} showSuccessRate={true} />
					</CardContent>
				</Card>
			)}
