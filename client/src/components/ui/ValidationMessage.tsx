...
export const ValidationMessage: FC<ValidationMessageProps> = ({
  ...
  return (
    ...
    >
      {/* Status Icon */}
      {getStatusIcon() && (
        <div className={combineClassNames('flex-shrink-0', sizeClasses.icon, statusStyles.icon)}>
          {getStatusIcon()}
        </div>
      )}
      ...
      {showMessages && (
        <div className='flex-1 space-y-1'>
          ...
          {status === 'valid' && successMessage && (
            <p className={combineClassNames(sizeClasses.message, statusStyles.message)}>
              {successMessage}
            </p>
          )}
...
export const ValidationError: FC<
  Omit<ValidationMessageProps, 'status'> & { errors: string[] }
> = props => <ValidationMessage {...props} status='invalid' />;

export const ValidationSuccess: FC<
  Omit<ValidationMessageProps, 'status'> & { message: string }
> = ({ message, ...props }) => (
  <ValidationMessage {...props} status='valid' successMessage={message} />
);

export const ValidationWarning: FC<
  Omit<ValidationMessageProps, 'status'> & { warnings: string[] }
> = props => <ValidationMessage {...props} status='warning' />;

export const ValidationLoading: FC<Omit<ValidationMessageProps, 'status'>> = props => (
  <ValidationMessage {...props} status='validating' />
);
