...
// Wrapper for GoogleIcon to match LucideProps interface
const GoogleIconWrapper: ComponentType<LucideProps> = ({ size, className, onClick }) => {
	const sizeMap: Record<number, ComponentSize> = {
		12: ComponentSize.XS,
		16: ComponentSize.SM,
		20: ComponentSize.MD,
		24: ComponentSize.LG,
		32: ComponentSize.XL,
		48: ComponentSize.XXL,
	};
	const iconSize = size && typeof size === 'number' ? sizeMap[size] || ComponentSize.MD : ComponentSize.MD;
	return <GoogleIcon size={iconSize} className={className} onClick={onClick} />;
};
