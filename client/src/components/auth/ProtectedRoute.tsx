 ...
 * @used_by AppRoutes.tsx for protecting sensitive routes
 */
...
export const ProtectedRoute = memo(function ProtectedRoute({ 
	...
	if (!isAuthenticated) {
		...
		// Store the attempted path for redirect after login (session only)
		// Note: This is temporary session data, not user state, so direct sessionStorage is appropriate
...
export const PublicRoute = memo(function PublicRoute({ 
	...
	const { isAuthenticated } = useSelector((state: RootState) => state.user) as { isAuthenticated: boolean };
	const location = useLocation();
