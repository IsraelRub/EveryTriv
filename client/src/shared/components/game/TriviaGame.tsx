import { TriviaAnswer, TriviaGameProps } from '../../types';
import { motion } from 'framer-motion';
import { fadeInVariants, popVariants, staggerContainer, PulseEffect } from '../animations';
import { useParticleSystem } from '../../hooks/useAdvancedAnimations';
...
export default function TriviaGame({ trivia, selected, onAnswer }: TriviaGameProps) {
	...
	// Enhanced answer handler with logging
	const handleAnswerClick = (index: number) => {
		if (selected !== null) return; // Already answered
		
		logger.user(`🎯 Answer selected`, {
			questionText: trivia.question.substring(0, 100) + (trivia.question.length > 100 ? '...' : ''),
			answerIndex: index,
			answerText: trivia.answers[index].text.substring(0, 50),
