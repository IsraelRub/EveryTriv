...
import { useUnifiedUserAnalytics, useDifficultyStats, usePopularTopics } from '../../hooks/api';
import { useRealTimeAnalytics, useAnalyticsExport } from '../../hooks/api/useAnalyticsDashboard';
...
export function AnalyticsView() {
	...
	// Use analytics hooks
	const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useUnifiedUserAnalytics();
	...
	return (
		...
		>
			<div className="flex items-center justify-between">
				...
				<div className={`text-3xl p-3 rounded-full bg-gradient-to-r ${colorClasses[color]} text-white`}>
					{icon}
				</div>
			</div>
		</motion.div>
...
function TopicsChart({ topicsPlayed }: { topicsPlayed: Record<string, number> }) {
	const topics = Object.entries(topicsPlayed).sort((a, b) => b[1] - a[1]).slice(0, 5);
...
function DifficultyChart({ difficultyBreakdown }: { difficultyBreakdown: Record<string, { correct: number; total: number }> }) {
			...
			{analyticsData.performance.averageGameTime && (
				<div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
					<div>
						...
						<p className="text-sm text-gray-600">{analyticsData.performance.averageGameTime} דקות</p>
					</div>
					<div className="text-blue-500">
						<svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
