...
import type { CompleteUserAnalytics, DifficultyBreakdown, TopicsPlayed } from '@shared/types';
...
import { ButtonVariant, CardVariant, ComponentSize, Spacing, USER_DEFAULT_VALUES } from '../../constants';
import {
	useAnalyticsExport,
	useDifficultyStats,
	usePopularTopics,
...
import { CardMetricProps, ComparisonCardProps } from '../../types';
			...
			>
				<div className='max-w-7xl mx-auto'>
					<div className='animate-pulse'>
						...
						<GridLayout variant='balanced' gap={Spacing.LG} className='mb-8'>
							{[1, 2, 3, 4].map(i => (
	...
	if (analyticsError) {
		return (
			<main
				...
				className='min-h-screen bg-gradient-to-br from-red-50 to-pink-50 p-6'
			>
				<div className='max-w-7xl mx-auto'>
	...
	return (
		...
		>
			<div className='max-w-7xl mx-auto'>
						...
						{(['week', 'month', 'all'] as const).map(filter => (
							<button
								...
								className={`px-4 py-2 rounded-lg transition-colors ${
									timeFilter === filter ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
								}`}
							>
				...
				<GridLayout variant='balanced' gap={Spacing.LG} className='mb-8'>
					<Card variant={CardVariant.WHITE}>
						<CardHeader>
							<CardTitle className='text-lg font-bold text-gray-900'>Performance Analysis</CardTitle>
						</CardHeader>
						<CardContent>
							<div className='space-y-3'>
								...
								<div className='flex justify-between items-center'>
									...
									<span className='font-semibold text-gray-900'>{analyticsData.performance.learningCurve ?? 0}%</span>
								</div>
								<div className='flex justify-between items-center'>
					...
					<Card variant={CardVariant.WHITE}>
						<CardHeader>
							<CardTitle className='text-lg font-bold text-gray-900'>Topics</CardTitle>
						</CardHeader>
						<CardContent>
							<div className='space-y-3'>
								{analyticsData.performance.strongestTopic && (
									<div className='flex justify-between items-center'>
										...
										<span className='font-semibold text-green-600'>{analyticsData.performance.strongestTopic}</span>
									</div>
								)}
								{analyticsData.performance.weakestTopic && (
									<div className='flex justify-between items-center'>
										<span className='text-sm text-gray-600'>To improve</span>
										<span className='font-semibold text-red-600'>{analyticsData.performance.weakestTopic}</span>
									</div>
								)}
								<div className='flex justify-between items-center'>
						...
						<CardContent>
							<div className='space-y-3'>
								<div className='flex justify-between items-center'>
									...
									<span className='font-semibold text-gray-900'>{analyticsData.game.correctAnswers}</span>
								</div>
								<div className='flex justify-between items-center'>
								...
								<div className='flex justify-between items-center'>
									...
									<span className='font-semibold text-gray-900'>{analyticsData.basic.accountAge} days</span>
								</div>
							</div>
						</CardContent>
					</Card>
				...
				<GridLayout variant='balanced' gap={Spacing.LG} className='mb-8'>
					<Card variant={CardVariant.WHITE}>
						<CardHeader>
							<CardTitle className='text-xl font-bold text-gray-900'>Scoring System</CardTitle>
						</CardHeader>
						<CardContent>
							<ScoringSystem
				...
				<GridLayout variant='content' gap={Spacing.LG} className='mb-8'>
					{/* Topics Performance */}
					<Card variant={CardVariant.WHITE}>
						<CardHeader>
							<CardTitle className='text-xl font-bold text-gray-900'>Performance by topic</CardTitle>
						</CardHeader>
						<CardContent>
							{topicsLoading ? (
					...
					<Card variant={CardVariant.WHITE}>
						<CardHeader>
							<CardTitle className='text-xl font-bold text-gray-900'>Breakdown by difficulty level</CardTitle>
						</CardHeader>
						<CardContent>
							{difficultyLoading ? (
								<div role='status' aria-live='polite' className='animate-pulse'>
				...
				{/* Real-time Analytics */}
				{realTimeAnalytics && (
					<Card variant={CardVariant.WHITE} className='mb-8'>
						<CardHeader>
							<CardTitle className='text-xl font-bold text-gray-900'>Real-time data</CardTitle>
						</CardHeader>
						<CardContent>
							<GridLayout variant='balanced' gap={Spacing.MD}>
								<div className='text-center p-4 bg-green-50 rounded-lg'>
									<div className='text-2xl font-bold text-green-600'>{realTimeAnalytics.game?.totalGames ?? 0}</div>
									<div className='text-sm text-green-700'>Games today</div>
								</div>
								<div className='text-center p-4 bg-blue-50 rounded-lg'>
									<div className='text-2xl font-bold text-blue-600'>{realTimeAnalytics.game?.correctAnswers ?? 0}</div>
				...
				<CardContent>
					<div className='flex items-center justify-between'>
						...
						<div className={`p-3 rounded-full bg-gradient-to-r ${colorClasses[color]} text-white`}>
							<Icon name={icon} size={ComponentSize.XL} color='white' />
						</div>
					</div>
				</CardContent>
			</Card>
...
function ComparisonCard({ title, userValue, averageValue, unit, higherIsBetter }: ComparisonCardProps) {
	...
	return (
		<Card variant={CardVariant.GRAY} padding={Spacing.SM}>
			<CardContent>
				...
				<div className='space-y-1'>
					...
					<div className='flex justify-between items-center'>
						...
						<span className={`text-xs font-medium ${getStatusColor()}`}>{Math.abs(percentageDiff).toFixed(1)}%</span>
					</div>
				</div>
			</CardContent>
		</Card>
							...
							<div className='flex-1'>
								<div className='bg-gray-200 rounded-full h-2'>
									<motion.div
										...
										aria-valuemin={0}
										aria-valuemax={100}
										aria-label={`${difficulty} difficulty: ${Math.round(successRate)}% success rate`}
									/>
								</div>
							</div>
						</div>
		...
		<div className='space-y-4'>
			<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
				...
				<div className='text-green-500'>
					<Icon name='checkcircle' size={ComponentSize.MD} color='success' />
				</div>
			</div>
			...
			<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
				...
				<div className='text-blue-500'>
					<Icon name='calendar' size={ComponentSize.MD} color='info' />
				</div>
			</div>
			...
			<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
				<div>
					...
					<p className='text-sm text-gray-600'>{analyticsData.performance.improvementRate.toFixed(1)}%</p>
				</div>
				<div className='text-purple-500'>
					<Icon name='zap' size={ComponentSize.MD} color='accent' />
				</div>
			</div>
			...
			{analyticsData.performance.averageGameTime && (
				<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
					...
					<div className='text-blue-500'>
						<Icon name='clock' size={ComponentSize.MD} color='info' />
					</div>
				</div>
			)}
			...
			{analyticsData.performance.consistencyScore && (
				<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
					...
					<div className='text-green-500'>
						<Icon name='list' size={ComponentSize.MD} color='success' />
					</div>
				</div>
			)}
			...
			{analyticsData.performance.learningCurve && (
				<div className='flex items-center justify-between p-4 bg-gray-50 rounded-lg'>
					...
					<div className='text-orange-500'>
						<Icon name='barchart' size={ComponentSize.MD} color='warning' />
					</div>
				</div>
			)}
		</div>
