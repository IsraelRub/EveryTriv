import { CONTACT_INFO, PAYMENT_CONTENT, PAYMENT_FEATURES, VALID_GAME_MODES } from 'everytriv-shared/constants';
import type { PointPurchaseOption, UrlResponse } from 'everytriv-shared/types';
...
import { FadeInDown,
	FadeInRight, FadeInUp, HoverScale, ScaleIn, SlideInUp, StaggerContainer } from '../../components/animations';
...
import { setPointBalance } from '../../redux/features';
...
export default function PaymentView() {
	...
	const { pointBalance } = useSelector((state: RootState) => state.user);
	...
	const handlePurchase = async () => {
		...
		const operationId = 'point-purchase';
		...
		try {
			...
			storageService.setItem('purchase-attempt', {
			...
			purchaseMutation.mutate(selectedPackage, {
				...
				onError: (err: Error) => {
					setPurchaseStatus('invalid');
					audioService.play(AudioKey.ERROR);
					logger.payment('Purchase failed', { error: err.message, packageId: selectedPackage });
					errorOperation(operationId, err.message);
					setPurchasing(false);
				},
			});
		} catch (err: unknown) {
			setPurchaseStatus('invalid');
			audioService.play(AudioKey.ERROR);
			logger.paymentFailed('payment_intent', err instanceof Error ? err.message : String(err), { 
				packageId: selectedPackage 
			});
			errorOperation(operationId, err instanceof Error ? err.message : 'Purchase failed');
			setPurchasing(false);
		}
	};
	...
	if (success) {
		return (
			<div className='min-h-screen pt-20 pb-12 px-4 flex items-center justify-center'>
				<ScaleIn className='text-center max-w-md'>
					...
					<div className='glass rounded-xl p-6 mb-8'>
						...
						<div className='space-y-2 text-left text-slate-300'>
							<div className='flex justify-between'>
								...
								<span className='font-semibold text-white'>{pointBalance.total_points}</span>
							</div>
							<div className='flex justify-between'>
								<span>Free Questions:</span>
								<span className='text-green-400'>{pointBalance.free_questions}</span>
							</div>
							<div className='flex justify-between'>
								<span>Purchased Points:</span>
								<span className='text-blue-400'>{pointBalance.purchased_points}</span>
							</div>
						</div>
					</div>
					...
					<CardGrid columns={3} gap='lg' className='mb-8'>
						{Array.isArray(packages) && packages.length > 0 ? (
							packages.map((pkg, index) => (
								<FadeInRight key={pkg.id} delay={0.1 + index * 0.1}>
									<div
