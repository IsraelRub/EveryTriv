import { CONTACT_INFO, PAYMENT_CONTENT, PAYMENT_FEATURES, VALID_GAME_MODES } from '@shared';
import type { PointPurchaseOption, UrlResponse, SubscriptionPlans as SubscriptionPlansType, SubscriptionData } from '@shared';
import { formatCurrency } from '@shared';
import { useEffect, useState, useMemo } from 'react';
import { useAppDispatch, useAppSelector } from '../../hooks/layers/utils';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { fadeInDown, fadeInRight, fadeInUp, scaleIn, slideInUp, createStaggerContainer } from '../../components/animations';
...
import { useCreateSubscription, useCancelSubscription } from '../../hooks/api';
...
export default function PaymentView() {
	...
	const features = useMemo(() => [
		...
		{
			icon: <Icon name={PAYMENT_FEATURES.SUPPORT.icon} size='2xl' color={PAYMENT_FEATURES.SUPPORT.color} />,
		...
		try {
			...
			clientLogger.performance('point_purchase', duration, { success: true });
				...
				onError: (err: Error) => {
					setPurchaseStatus('invalid');
					audioService.play(AudioKey.ERROR);
					clientLogger.payment('Purchase failed', { error: err.message, packageId: selectedPackage });
					const duration = performance.now() - startTime;
					clientLogger.performance('point_purchase', duration, { success: false, error: err.message });
			...
			setPurchaseStatus('invalid');
			audioService.play(AudioKey.ERROR);
			clientLogger.paymentFailed('payment_intent', err instanceof Error ? err.message : String(err), { 
				packageId: selectedPackage 
			});
			const duration = performance.now() - startTime;
			clientLogger.performance('point_purchase', duration, { success: false, error: err instanceof Error ? err.message : 'Purchase failed' });
			clientLogger.userError('Purchase failed', { error: err instanceof Error ? err.message : 'Purchase failed' });
			setPurchasing(false);
		}
	};
	...
	return (
		<Container size='xl' className='min-h-screen flex flex-col items-center justify-start p-4 pt-20'>
			<Section padding='xl' className='w-full space-y-8'>
				...
				{/* Tabs */}
				<motion.div variants={fadeInUp} initial='hidden' animate='visible' exit='exit' className='mb-8'>
								...
								<div className='space-y-4'>
									{currentSubscription && currentSubscription.plan !== 'free' ? (
										<div className='bg-green-500/10 border border-green-400/30 rounded-lg p-4'>
											<h3 className='text-lg font-semibold text-green-300 mb-2'>Active Subscription</h3>
											<div className='grid grid-cols-1 md:grid-cols-3 gap-4 text-sm'>
												<div>
													<span className='text-slate-400'>Plan:</span>
													<span className='text-white ml-2'>{String(currentSubscription.plan || 'Unknown')}</span>
												</div>
												<div>
													<span className='text-slate-400'>Status:</span>
													<span className='text-green-400 ml-2'>{String(currentSubscription.status || 'Unknown')}</span>
												</div>
												<div>
													<span className='text-slate-400'>Next Billing:</span>
													<span className='text-white ml-2'>{String(currentSubscription.nextBillingDate || 'Unknown')}</span>
												</div>
											</div>
										...
										<div className='bg-slate-500/10 border border-slate-400/30 rounded-lg p-4'>
											<h3 className='text-lg font-semibold text-slate-300 mb-2'>No Active Subscription</h3>
											<p className='text-slate-400 text-sm mb-4'>
												Subscribe to unlock unlimited questions and premium features.
											</p>
											<Button
												variant='primary'
												onClick={() => createSubscription.mutate({ plan: 'premium', billingCycle: 'monthly' })}
				...
				{activeTab === 'subscription' && currentSubscription && currentSubscription.plan !== 'free' && (
					<motion.div variants={fadeInUp} initial='hidden' animate='visible' exit='exit' className='mb-8'>
						<div className='bg-green-500/20 border border-green-400/30 rounded-lg p-4 text-center'>
							...
							<h3 className='text-lg font-semibold text-green-400 mb-1'>
								Active {String(currentSubscription.plan || 'Unknown').charAt(0).toUpperCase() + String(currentSubscription.plan || 'Unknown').slice(1)} Subscription
							</h3>
							...
							<CardGrid columns={3} gap='lg' className='mb-8'>
								{Array.isArray(packages) && packages.length > 0 ? (
												...
												<div className='text-center'>
													...
													<div className='space-y-3 mb-6'>
														<div className='flex items-center text-slate-300'>
															<Icon name='checkcircle' size='sm' color='success' className='mr-3 flex-shrink-0' />
									...
									<div className='col-span-full text-center py-12'>
										...
										<h3 className='text-xl font-semibold text-white mb-2'>{PAYMENT_CONTENT.PACKAGES.noPackagesTitle}</h3>
				...
				{selectedPackage && (
					...
					>
						<Section background='glass' padding='lg' className='rounded-lg'>
							<h2 className='text-2xl font-bold text-white mb-6 text-center'>{PAYMENT_CONTENT.PAYMENT.title}</h2>
				...
				>
					<Section background='glass' padding='lg' className='rounded-lg'>
						<h2 className='text-2xl font-bold text-white mb-6 text-center'>{PAYMENT_CONTENT.FEATURES.title}</h2>
						<motion.div 
