...
import type { 
	ApiResponse
} from '@shared/types/infrastructure/api.types';
import type { LanguageValidationOptions, LanguageValidationResult } from '@shared/types/language.types';
...
export interface ClientApiService {
	...
	// User search methods
	searchUsers(query: string, limit?: number): Promise<{ query: string; results: User[]; totalResults: number }>;
	...
	validateLanguage(text: string, options?: {
		language?: string;
	...
	getGlobalLeaderboard(limit?: number, offset?: number): Promise<{ leaderboard: LeaderboardEntry[]; pagination: { total: number; limit: number; offset: number } }>;
	getLeaderboardByPeriod(period: 'weekly' | 'monthly' | 'yearly', limit?: number): Promise<LeaderboardEntry[]>;
