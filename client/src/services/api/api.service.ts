...
import { API_BASE_URL, HTTP_CLIENT_CONFIG, HTTP_STATUS_CODES } from '@shared/constants/infrastructure/http.constants';
...
class ApiService implements ClientApiService {
	...
	private async retryRequest<T>(
		...
		try {
			...
			const shouldRetry = 
				...
				((apiError as ApiError & { isServerError?: boolean }).isServerError || apiError.statusCode === 0);
	...
	private async handleResponse<T>(response: Response): Promise<ApiResponse<T>> {
		if (!response.ok) {
			...
			} as ApiError & { isServerError: boolean; isClientError: boolean; url: string; method: string };
		}
		...
		if (responseData && typeof responseData === 'object' && (responseData as { success?: unknown }).success !== undefined) {
	...
	async login(credentials: AuthCredentials): Promise<AuthResponse> {
		...
		if ((response.data as { refresh_token?: string }).refresh_token) {
			await storageService.set(CLIENT_STORAGE_KEYS.REFRESH_TOKEN, (response.data as { refresh_token?: string }).refresh_token!);
		}
	...
	async getUserPercentile(): Promise<{ percentile: number; rank: number; totalUsers: number }> {
		const response = await this.get<{ percentile: number; rank: number; totalUsers: number }>('/leaderboard/user/percentile');
		return response.data;
	}
	...
	async createSubscription(plan: string, billingCycle?: string): Promise<SubscriptionData> {
		const response = await this.post<SubscriptionData>('/subscription/create', { plan, billingCycle: billingCycle || '' });
		return response.data;
	}
	...
	async getTrivia(request: TriviaRequest): Promise<TriviaQuestion> {
		...
		if (!request || !request.topic || !request.difficulty) {
		  ...
		  if (request.question_count && (Number(request.question_count) < 1 || Number(request.question_count) > 50)) {
	...
	async validateCustomDifficulty(customText: string): Promise<CustomDifficultyValidationResponse> {
		...
		const response = await this.post<CustomDifficultyValidationResponse>('/game/validate-custom', request);
		return response.data;
	}
	 ...
	 */
	async validateLanguage(text: string, options?: {
		language?: string;
	...
	// User search methods
	async searchUsers(query: string, limit?: number): Promise<{ query: string; results: User[]; totalResults: number }> {
		const response = await this.post<{ query: string; results: User[]; totalResults: number }>('/users/search', { query, limit: limit || 0 });
		return response.data;
	}
	...
	async updateUserRanking(): Promise<{ success: boolean; message: string }> {
		const response = await this.post<{ success: boolean; message: string }>('/leaderboard/user/update');
		return response.data;
	}
	 ...
	 */
	async getLeaderboardByPeriod(period: 'weekly' | 'monthly' | 'yearly', limit: number = 100): Promise<LeaderboardEntry[]> {
		// Validate period
		...
		const response = await this.get<LeaderboardEntry[]>(`/leaderboard/period/${period}?limit=${limit}`);
		return response.data;
	}

	async getGlobalLeaderboard(limit: number = 100, offset: number = 0): Promise<{ leaderboard: LeaderboardEntry[]; pagination: { total: number; limit: number; offset: number } }> {
		const response = await this.get<{ leaderboard: LeaderboardEntry[]; pagination: { total: number; limit: number; offset: number } }>(`/leaderboard/global?limit=${limit}&offset=${offset}`);
		return response.data;
	}
	 ...
	 * Get payment history
	 */
	async getPaymentHistory(limit?: number, offset?: number): Promise<{
		payments: Array<{
			id: string;
			amount: number;
			currency: string;
