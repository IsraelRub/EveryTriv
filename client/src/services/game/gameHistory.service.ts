...
class ClientGameHistoryService {
  ...
  async saveGameResult(gameData: GameHistoryRequest): Promise<GameHistoryEntry> {
    try {
      ...
      logger.gameStatistics('Game result saved successfully', { gameId: gameHistory.id });
      return gameHistory;
    } catch (error) {
      logger.gameError('Failed to save game result', { error, gameData });
      throw error;
    }
  }
  ...
  async getUserGameHistory(limit: number = 20, offset: number = 0): Promise<GameHistoryEntry[]> {
    try {
      ...
      logger.gameError('Failed to get user game history', { error, limit, offset });
      throw error;
    }
  }
  ...
  async getLeaderboard(limit: number = 10): Promise<LeaderboardEntry[]> {
    try {
      ...
      logger.gameError('Failed to get leaderboard', { error, limit });
      throw error;
    }
  }
  ...
  async getUserRank(): Promise<UserRankData | null> {
    try {
      ...
      logger.gameError('Failed to get user rank', { error });
      throw error;
    }
  }
  ...
  async getUserStats(): Promise<UserStatsData> {
    try {
      ...
      logger.gameError('Failed to get user statistics', { error });
      throw error;
    }
  }
  ...
  async getGameById(gameId: string): Promise<GameHistoryEntry> {
    try {
      logger.gameStatistics('Getting game by ID', { gameId });
      ...
      logger.gameError('Failed to get game by ID', { error, gameId });
      throw error;
    }
  }
}
