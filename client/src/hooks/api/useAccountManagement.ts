...
export const useUpdateUserField = () => {
	...
	return useMutation({
		...
		onSuccess: (data, { field, value }) => {
			// Update Redux state for HOCs consistency
			if (data && typeof data === 'object' && 'user' in data && (data as { user: UserWithSubscription }).user && user) {
...
export const useUpdateUserCredits = () => {
	...
	return useMutation({
		...
		onSuccess: (data, { userId }) => {
			...
			if (user && user.id === userId && data && typeof data === 'object' && 'user' in data && (data as { user: UserWithSubscription }).user) {
				const userData = (data as { user: UserWithSubscription }).user;
				dispatch(setUser(userData));
			}
