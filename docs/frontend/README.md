# Frontend - EveryTriv

אינדקס ראשי לתיעוד Frontend של EveryTriv.

## מבנה ואדריכלות

- **[VIEWS.md](./VIEWS.md)** - דפי האפליקציה
- **[COMPONENTS.md](./COMPONENTS.md)** - רכיבי UI
- **[services/SERVICES.md](./services/SERVICES.md)** - שירותי Frontend
  - **[services/interceptors/INTERCEPTORS.md](./services/interceptors/INTERCEPTORS.md)** - Interceptors
- **[HOOKS.md](./HOOKS.md)** - ארכיטקטורת Hooks (כולל React Query)
- **[REFRESH_INVALIDATION.md](./REFRESH_INVALIDATION.md)** - רענון ואינברידציה (Refresh / invalidation)
- **[REDUX.md](./REDUX.md)** - ניהול מצב (Redux Toolkit)
- **[ROUTING.md](./ROUTING.md)** - מערכת ניתוב

## קבועים, טיפוסים ופונקציות שירותיות

- **[CONSTANTS.md](./CONSTANTS.md)** - קבועים
- **[TYPES.md](./TYPES.md)** - טיפוסים
- **[UTILS.md](./UTILS.md)** - פונקציות שירותיות

## מערכות עיצוב ואנימציה

- **[DESIGN_SYSTEM.md](./DESIGN_SYSTEM.md)** - מערכת עיצוב
- **[ANIMATION_SYSTEM.md](./ANIMATION_SYSTEM.md)** - מערכת אנימציות
- **[AUDIO_SYSTEM.md](./AUDIO_SYSTEM.md)** - מערכת אודיו

## קישורים כלליים

- [ארכיטקטורה כללית](../ARCHITECTURE.md)
- [דיאגרמות](../DIAGRAMS.md)
- [API Reference](../backend/API_REFERENCE.md)

